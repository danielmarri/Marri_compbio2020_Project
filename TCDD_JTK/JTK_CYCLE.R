#A function with input variables been Phases and periods to generate the output 2*pi*phase/periods
#This function is used to change hours to radians
hours.to.radians <- function(phases, periods) {
  2 * pi * phases / periods
}


#Another function to take into account angles, period and change radians to hours.

radians.to.hours <- function(angles, periods=NULL) {
  hours <- angles * 12 / pi
  hours <- hours %% 24 #This is a modular division of 24 hrs
  ifelse(is.null(periods), hours, hours * mean(periods) / 24)#If the period is given as an input 
                                                             #variable execute hours * mean(periods) / 24
}

#Another function to take into account angles,to calculate circular mean.
